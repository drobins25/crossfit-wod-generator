...
type LiftT = { focus:MuscleGroup, move:string, scheme:string, minutes:number, difficulty:number } | null
...
export function BoardProvider({ children }:{ children: React.ReactNode }){
  ...
  function generateAll(){
    const minutesLift = Math.round(workout*split); const minutesHiit = workout - minutesLift
    ...
    setLift(L); setHiit(H); setWarm(P.warmup); setCool(P.cooldown); setPrimary(P.primary)
  }
  function regenLift(){
  ...
  const value: Ctx = { date,setDate, split,setSplit, workout,setWorkout, equipSel,setEquipSel, soreSel,setSoreSel, lift,hiit,warm,cool,primary, generateAll, regenLift, regenHiit }
  return <C.Provider value={value}>{children}</C.Provider>
}

export function useBoard(){ return useContext(C) }
